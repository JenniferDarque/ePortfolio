...
int main(int argc, char* argv[])
{
	...
	glfwMakeContextCurrent(g_Window);   // Required before glewInit
	...
	// Get actual framebuffer size and pass to SceneManager
	int width, height;
	...
	// register callbacks AFTER g_SceneManager is valid
	glfwSetCursorPosCallback(g_Window, [](GLFWwindow* window, double xpos, double ypos) {
		if (g_SceneManager) g_SceneManager->ProcessMouseMovement(xpos, ypos);
		});
	glfwSetScrollCallback(g_Window, [](GLFWwindow* win, double xoffset, double yoffset) {
		if (g_SceneManager) g_SceneManager->ProcessMouseScroll(yoffset);
		});
